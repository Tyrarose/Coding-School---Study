...
def status(player,enemy): # Print player status
    decorated(f"               <{player.name}>'s Turn               "+
    f"\n<{player.name}> Stat: # Health: {player.health}     # Attack: {player.attack}     # Armor: {player.armor}" +
    f"\n<{enemy.name}> Stat: # Health: {enemy.health}     # Attack: {enemy.attack}     # Armor: {enemy.armor}")
...
def startGame(player1,player2): 
    ...
    while(current_player.health >= 0):
        ...
        if(current_player.health <= 0): # To end the while loop
            ...
            status(current_player,current_enemy) # Display status
            ...
            current_enemy = player1 # Switch enemy
            ...
            current_enemy = player2 # Switch enemy
